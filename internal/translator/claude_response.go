		...
		if delta, ok := raw["delta"].(map[string]any); ok {
			if stopReason, ok := delta["stop_reason"].(string); ok && stopReason != "" {
				...
				case "end_turn", "stop_sequence":
					finishReason = "stop"
				case "refusal":
					// A refusal is a blocked turn, not a clean stop: without this
					// mapping the client sees finish_reason "stop" with an empty
					// message, indistinguishable from a real answer
					// (parity with decolua/9router#4210).
					finishReason = "content_filter"
				}
				// A refusal carries no content blocks at all. Surface Anthropic's
				// own explanation as message text so the client shows *why* the
				// turn is empty instead of a blank reply.
