...
**Token Saver Page (RTK, Headroom, Caveman, Ponytail in one place):**
- `internal/handlers/dashboard/ui/index.html` — the four token savers were four checkboxes buried in a Settings sub-block, and Headroom had no UI at all despite the server exposing its whole lifecycle. They now have a dedicated sidebar page (`#token-saver`) mirroring upstream's `/dashboard/token-saver`: RTK, Headroom with a live status badge and a Manage modal (start/restart/stop, extras `[code]`/`[ml]`, install log, user-message switch), Caveman and Ponytail with segmented level pickers, and the injection guard. Every control saves on change — a toggle that looks applied but is not is worse than no toggle — and re-reads the server after each write so a normalized or rejected value cannot leave the UI lying.
- `internal/handlers/dashboard/dashboard.go` — `GET /api/dashboard/settings` now reports the Headroom state (`enabled`, `url`, `timeoutMs`, `codeAware`, `kompress`, `compressUserMessages`) and the level vocabularies (`cavemanLevels`, `ponytailLevels`), so the dropdowns are built from what the server accepts instead of a hardcoded list. `POST` accepts and persists all of them, validating levels and treating the switch and the level as independent fields (a level-only patch used to be dropped).
- `internal/db/settings.go` — `headroomEnabled` and `headroomCompressUserMessages` join the typed settings; both are registered in `knownSettingsKeys` so a round-trip through a VansRouter database cannot lose them.
- `internal/handlers/router.go` — `/headroom/` joins the session-cookie allowlist. The page drives start/stop/extras from the browser, which holds only the httpOnly cookie, so without this the badge read "Not installed" and every button 401'd.
- The old Settings sub-block is gone rather than left as a duplicate: two places to toggle the same switch is two places to disagree, and the page it now lives on is reachable from the sidebar.
- `internal/handlers/dashboard/ui/index.html` — card layout: the five cards have very different heights, so pairing them by height (RTK+Caveman, Ponytail+Guard) and giving Headroom a full-width row keeps every row tight, with badges and level pickers pinned to each card's foot so they line up across a row.

**Per-Request Token-Saver Bypass (`X-9Router-Token-Saver: off`):**
- `internal/tokensaver/levels.go` — port of the reference's `TOKEN_SAVER_HEADER`. A client that needs its body verbatim (a benchmark, a debugging session, a prompt-cache-sensitive workload) should not have to change a global setting and affect every other caller. The decision is made once at the HTTP boundary and travels on the context, so no saver stage re-parses the header. Value matching is case-insensitive and only `off` triggers it.
- `internal/handlers/chat/chat.go` — both entry points (`/v1/chat/completions`, `/v1/messages`) mark the context. Verified against a live server with a mock upstream: a plain request arrives with the Caveman system prompt injected, the same request with the header arrives byte-identical.

**Headroom Wired Into the Request Path:**
- `internal/tokensaver/headroom.go` — the `/v1/compress` client: posts the conversation, returns the compressed messages and the proxy's token counters. Fail-open on every error path (no URL, timeout, non-2xx, truncated or empty reply, unreachable host) because Headroom is an optional sidecar and turning an unreachable one into a failed chat would make it a liability. The timeout defaults to the reference's 3000 ms rather than meaning "no deadline".
- `internal/handlers/chat/fallback.go` — `applyTokenSavers` runs the pipeline in the reference's order: injection scan, RTK, Headroom, then the Caveman and Ponytail injections. Headroom goes before the injections because it rewrites the conversation while they append to it; compressing afterwards would let the proxy shorten the very style prompt the operator asked for. Claude-format bodies are translated to OpenAI shape and back (the proxy only understands chat messages), and Responses-API bodies carrying tool/reasoning items are skipped rather than half-converted. Verified live: the proxy was called at `/v1/compress`, the upstream received the compressed content, and pointing Headroom at a dead port left the body untouched while the request still succeeded.

**Caveman and Ponytail Levels Now Match the Reference:**
- `internal/tokensaver/prompts.go`, `internal/tokensaver/levels.go` — the Go dashboard offered `light`/`medium`/`compact`, none of which the prompt lookup had, so two of the three caveman options and one of the two ponytail options silently applied a different style than the label promised. The vocabularies are now `lite`/`full`/`ultra` for both, exposed to the UI from one place. The legacy spellings are kept as read-path aliases (they resolve to `lite` and `full`) so a database written by the older build does not change the operator's saved style on upgrade, while the write path rejects them.
- `internal/handlers/shared/tokensaver.go` — the config normalizes on `Set`, which is the path `main.go` uses at startup.
...
**Freebuff "Add account" Never Completed — Wrong Host:**
- `internal/handlers/oauth/freebuff_oauth.go` — the device login targeted `freebuff.com`. The released CLI (codebuff 1.0.688, confirmed from its binary) uses `https://www.codebuff.com`, and the two are separate services: different GitHub OAuth applications (`Ov23liBmPZjygPRVpLQs` against `Ov23liw1KEfqvhsQV2Mc`) and no `/api/v1` on freebuff.com, which answers 404 where codebuff.com answers 401 "Invalid Codebuff API key". The failure was silent rather than loud: freebuff.com's `/api/auth/cli/code` still returns 200 with a real `loginUrl`, so the operator was sent to a genuine sign-in page, signed in, and the dashboard then polled until it timed out — the `auth_code` belonged to an account on the other service, so this host's status RPC never bound it.
- The fingerprint now uses the `codebuff-cli-<8>` shape the released client sends instead of a bare random string. The backend accepted both, but that leniency is undocumented and the real shape costs nothing.
- `expiresAt` from the code response is forwarded unchanged instead of being clamped to 5 minutes. The status RPC is asked about the code this server issued; the 5-minute deadline is the CLI's poll bound, so it is now reported separately as `pollTimeoutMs` and the dashboard uses that for its loop. Previously the clamped value was sent to the status endpoint, which is not what the CLI does.
- Verified against the live API: `/api/auth/cli/code` on `www.codebuff.com` returns 200, `/api/auth/cli/status` with our fingerprint triple returns 401 while pending (the documented in-progress answer), and `/api/v1/chat/completions` exists there while freebuff.com 404s.


**Antigravity Quota Panel Was Empty (`Quota API not implemented`):**
- `internal/quotatracker/antigravity.go` — port of the reference's `getAntigravityUsage()` (`open-sse/services/usage/google.js`). The tracker had no Antigravity handler at all, so the provider's Quota tab answered "Quota API not implemented for antigravity." for every account. It now reads both RPCs: `fetchAvailableModels` for per-model remaining fractions and `retrieveUserQuotaSummary` for the weekly/5h buckets.
- The tier gate is the part that cannot be dropped. A free-tier account has no 5h window: the per-model call still answers, but a missing `remainingFraction` reads as 0, so rendering those numbers shows a wall of exhausted models that are in fact usable. `loadCodeAssist` is read first and the per-model block is skipped for free (or unknown) tiers — unknown resolves to free, because that is the reading which does not invent an allowance.
- The model list is a BLOCKLIST, not an allow-list. Upstream carries a curated `importantModels` array, and it had gone stale: for one account here it named `gemini-3.8-flash-high/-medium/-low` (which the API no longer returns) while hiding `gemini-3.1-flash-lite`, `gemini-3.1-pro-high`, `gemini-3.5-flash-lite` and the three `-tiered` ids (which it does). The panel reported 9 model rows where the picker offered 18. The filter now lives in `internal/providers/antigravity_models.go` as `IsDiscoverableAntigravityModel` and is shared with the model picker, so the two can no longer disagree: a new id is callable by default, and only the known non-chat surfaces, retired ids and `chat_*`/`tab_*` internal slots are removed.
- Rows the API ships without a `displayName` (the `-tiered` ids) are humanised rather than printed raw, and a `displayName` shared by two ids (`gemini-3.1-pro-high` and its alias `gemini-pro-agent` both answer "Gemini 3.1 Pro (High)") is disambiguated with the id. Ids are walked in sorted order, so the suffix lands on the alias every time instead of on whichever id a randomised map walk visited first.
- Result on a live account: 12 model rows where there were 9, all with unique labels.
- `internal/handlers/dashboard/ui/index.html` — the quota bar was drawn as PERCENT USED, so a healthy allowance with nothing consumed rendered as an empty bar: the panel said "kosong" for exactly the accounts that had the most left. It now draws percent REMAINING and labels the number `(N% left)`, matching OmniRoute. Colour still tracks pressure (green healthy → amber → red), so the bar is full and green when nothing has been spent.
- `internal/quotatracker/antigravity.go` — the quota RPC was sent with a hardcoded `User-Agent: antigravity/1.0.0`, which is not a fingerprint any official client sends. The backend keys the catalogue on this header: measured against a live account, `ide/2.11.0` answers 33 models while the malformed string (and the CLI profile) answers 27. Six models — `gemini-3.7-flash-high/-medium/-low` and `gemini-3.8-flash-high/-medium/-low` — were therefore missing from the panel even though the picker listed them. The UA now comes from `providers.AntigravityUserAgent` for the connection's configured profile, so the panel and the picker cannot disagree. `Credentials` gains `ClientProfile` to carry it.
- Result on a live account: 18 model rows, matching the picker's 18 exactly.
- **The quota numbers themselves were wrong, and the cause was the host.** Google runs two Cloud Code endpoints that do not agree. Measured on one account, same token, same model, seconds apart: `daily-cloudcode-pa.googleapis.com` answered `remainingFraction: 0.734` for `gemini-3.1-pro-high` while `cloudcode-pa.googleapis.com` answered `1.0`. `cloudcode-pa` keeps reporting the Gemini family as full after the 5-hour window has been consumed, so a panel reading only that host showed a spent allowance as untouched — the "kuota tidak akurat" report. Weekly was affected too: 85.1% on daily against 100% on cloudcode-pa, and 85% is the figure OmniRoute displayed. The tracker now probes `daily` first and falls back to `cloudcode-pa`, mirroring `ANTIGRAVITY_RUNTIME_BASE_URLS`. A configured host outside that list is used alone, so an operator's local proxy is not bypassed and credentials are not sent to Google behind it.
- Verified against live accounts after the change: Gemini weekly 81-86%, Gemini 5h 72-80%, per-model `gemini-3.1-pro-high` 72-80% — all previously reported as 100%. Cross-checked against OmniRoute's own `quota_snapshots` table for the same connections, which agreed to two decimals (91.08% there, 91.1% here for `claude_gpt_weekly`).
- `internal/handlers/dashboard/ui/index.html` — the quota rows leaked onto every other provider tab. `showDetailTab` hid a hand-written list of panel ids that omitted `dt-quota`, so opening Quota and switching to Connections (or Capabilities, or Available Models) left the quota card rendered underneath. The hide list is now derived from the DOM (`[id^="dt-"]`), so a panel added to the markup cannot be forgotten again.
- The same two faults were in the ROUTING gate, not just the panel. `internal/handlers/chat/antigravity_quota.go` also read only `cloudcode-pa` and also sent `User-Agent: antigravity/1.0.0`. That path decides which models the router treats as exhausted, so a stale "full" reading there did not merely mislabel a row — it kept routing traffic to a model the account had spent, and hid exhaustion behind a healthy-looking number. Both paths now share `providers.AntigravityQuotaHosts` and `providers.AntigravityUserAgent`, so the panel and the gate cannot disagree.
- `internal/quotatracker/antigravity.go` — the session/weekly reconciliation from upstream: when every model in a family reports zero, that family's 5h row is marked spent with the latest model reset, because the summary can still report the window as partially full. The weekly row is left alone — a spent 5h window says nothing about the weekly allowance.
- `internal/handlers/dashboard/quota_handler.go` — the quota route now refreshes an expired credential before asking. Google answers a stale access token with an empty or 401 response rather than a useful error, so without this every account whose token had lapsed reported "no quota", indistinguishable from a healthy account with nothing left. Mirrors the reference's `refreshAndUpdateCredentials()`; a refresh failure is not fatal.
- `internal/providers/providers.go` — `antigravity` gains `UsageURL` (the weekly summary) and a new `SubscriptionURL` field for `loadCodeAssist`. The quota RPCs live on `cloudcode-pa`, not on the chat host.
- `internal/quotatracker/tracker.go` — `Quota` gains `displayName`, and the dashboard prefers it over the map key: `gemini_weekly` and `claude-sonnet-4-6` are stable identifiers, not labels.
- Verified against live accounts: 7 Antigravity connections now report a plan (`g1-pro-tier`), 22 rows each (4 family buckets + 18 models), with real weekly usage (3.7%, 8.9%, 38.7% across accounts) and per-model resets.


**Prompt Cache Held 92% of the Process Heap at a 0.5% Hit Rate:**
- `internal/promptcache/cache.go` — the cache bounded the number of ENTRIES (1000) but not the number of BYTES, while each entry was allowed up to 5 MB. The real ceiling was therefore ~5 GB, and a production deployment sat at 1000 entries holding 194 MB of response bodies — 92% of the process heap, on a 1 GB VPS. Eviction is now driven by a byte budget (16 MiB) as well as the entry count, a single response above the per-entry cap (512 KiB) is rejected rather than allowed to evict the whole cache, and `Clear`/replacement keep the running byte total correct. Diagnosed from a live heap profile: `bytes.growSlice` was 92.4% of `inuse_space`, reached through `cachingResponseWriter.Write`.
- Expired entries are now swept by the cache itself, once a minute. Eviction was previously driven only by capacity, so an idle server held every entry until new traffic pushed it out: the 15-minute TTL bounded reads but not memory, and a burst of large responses pinned them for as long as nothing else arrived.
- `internal/handlers/chat/prompt_cache.go` — the response writer's 5 MB buffer cap is replaced by the cache's own per-entry limit, so the buffer and the cache cannot disagree about what fits. `GET /api/cache/stats` now reports `bytes`/`maxBytes`: the entry count alone cannot tell an operator whether the cache is holding a few small responses or most of the heap, which is the failure this endpoint exists to make visible.
- `internal/handlers/chat/resolution.go` — entry capacity 1000 → 500, matching the smaller byte budget (the count is now a secondary limit that keeps the LRU list small, not the memory bound).
- Measured after: 260 distinct 400 KB responses retain 15.6 MiB (budget 16 MiB) with the heap steady at ~20 MiB; 40 responses of 900 KB — above the per-entry cap — are not buffered at all and leave both the heap and the cache unchanged.


**Console Log Resource Card Answered 401 Whenever Login Was Disabled:**
- `internal/middleware/auth_require_apikey.go` — with `requireLogin=false` the session group serves `/dashboard` and `/api/dashboard/*` to an anonymous caller, but the browser-facing endpoints the same page calls (`/api/system/metrics`, `/api/usage/stats`, the console-log routes) live in the engine group behind `RequireApiKeyUnlessDisabled`, which only ever accepted a session cookie or an API key. Login disabled means no cookie is ever issued, so every one of those routes answered 401 while the rest of the dashboard worked — reported as "Could not read resource metrics: HTTP 401" on a page whose Usage tab looked fine. The middleware now reads `requireLogin` in the same per-request settings read it already did and lets those paths through when it is off, using the same `sessionPaths` allowlist that scopes the cookie. The engine surface is unchanged: `/v1` is not on that list, so `POST /v1/chat/completions` still returns 401 in exactly this configuration (verified against a live server), and a caller that declares no paths gets no bypass at all.
- Verified end-to-end: anonymous `GET /api/system/metrics` 200 with login disabled and 401 with login required; the Console Log resource card renders live CPU/RAM/uptime values in a browser holding no cookie.
- `internal/middleware/auth_require_apikey_test.go` — the settings helper carries `requireLogin`, plus four tests pinning the new contract: the bypass opens dashboard paths, does not open the engine, is inert while login is required, and never fires without a declared path list.

**Usage by Model Table: Every Column Sortable:**
- `internal/handlers/dashboard/ui/index.html` — header `Usage by Model/Account/API Key/Endpoint` kini `th[data-sort]` dengan delegated click: klik = urutkan, klik lagi = balik arah. Kolom angka (Requests, token/cost) mulai terbesar-dulu, kolom teks (Model, Provider, Last Used) A-Z; indikator ▲/▼ pada kolom aktif. Pengurutan terjadi setelah agregat grup dihitung (`rows[]`), sehingga detail rows tetap menempel di bawah grupnya saat sort ulang; sort bertahan saat ganti periode/view, dan otomatis direset bila kolomnya hilang (mis. ganti Costs→Tokens). ISO `lastUsed` dibandingkan sebagai string — kronologis tanpa parsing. Field state mati `sortBy`/`sortOrder` (didefinisikan, tidak pernah dibaca) ikut dihapus.

**Usage Topology & Recent Requests Kaki Kartu Kini Sejajar:**
- `internal/handlers/dashboard/ui/index.html` — baris dua-kolom usage membungkus label+panel per kolom, jadi tiap kolom menghitung tinggi labelnya sendiri (header topology juga membawa toggle All/Used): di desktop bingkai Recent card turun 162px lebih rendah dari kotak topology (235→855 vs 273→693). Baris ini kini grid `grid-template-areas` 2×2 (baris label, baris panel) — terukur di 1440×900: kedua label 235→263, kedua panel 273→871, persis sejajar atas-bawah.
- `.usage-topology` desktop: `height:420px` → `height:auto; min-height:420px` agar ikut memanjang mengikuti tinggi baris; viewBox SVG berbasis jumlah provider, jadi tidak perlu render ulang saat resize. Layout satu kolom (<1024px) tetap urut label→panel→label→panel. Aturan `.card-ghost` yang tidak terpakai ikut dihapus.

**Daily Usage Rollup Was Keyed By UTC Day:**
- `internal/handlers/chat/usage.go` — `upsertDailyUsage` wrote `usageDaily` under `time.Now().UTC().Format("2006-01-02")` while the dashboard reads the day back through `PeriodCutoff("today")`, which opens at **local** midnight. Two writers of the same word disagreeing on the day is harmless for 17 hours and wrong for the rest: on UTC+7, every request arriving between 00:00 and 07:00 local was filed into yesterday's key, so the Today card showed an empty day while the request clearly counted — and the run before local 07:00 loaded yesterday's bucket a second time. Timestamps in `usageHistory` were never wrong; only the aggregate bucket boundary was.
- `internal/db/usage_query.go` — `dateKeyFor` exported as `DateKey` so the rollup writer and the read path call one function instead of two zone conventions. All in-package callers (`usage_stats.go` daily windows and chart buckets) migrated; behaviour unchanged there, since they already passed local-located times.
